...
class SpeechGPTInference:
    ...
    def preprocess(
        ...
        processed_parts = []
        for part in raw_text.split("is input:"): #判断wav输入
            if os.path.isfile(part.strip()) and os.path.splitext(part.strip())[-1] in [".wav", ".flac", ".mp4"]:
                processed_parts.append(self.s2u(part.strip(),noise = noise,merged=True)) #这个地方是处理音频
            # else:
    ...
    #             preprocessed_prompts.append(self.preprocess(prompt, noise)) #预处理
...
if __name__=='__main__':
    ...
    parser.add_argument("--model-name-or-path", type=str, default="./SpeechGPT-7B-cm")
    parser.add_argument("--lora-weights", type=str, default='./SpeechGPT-7B-com')
    ...
    prompt = str(f"prompt/output.wav")
    infer.forward([prompt])

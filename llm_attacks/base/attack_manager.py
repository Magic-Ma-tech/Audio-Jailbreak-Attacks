...
def get_embedding_matrix(model):
    ...
    elif isinstance(model._orig_mod.base_model.model, LlamaForCausalLM):
        return model._orig_mod.base_model.model.model.embed_tokens.weight
    else:
        raise ValueError(f"Unknown model type: {type(model)}")
...
def get_embeddings(model, input_ids):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        return model._orig_mod.base_model.model.model.embed_tokens(input_ids)
    else:
        raise ValueError(f"Unknown model type: {type(model)}")
...
class AttackPrompt(object):
    ...
    def logits(self, model, test_controls=None, return_ids=False):
        ...
        if pad_tok >= 0:
            attn_mask = (ids != pad_tok).type(ids.dtype).to(model.device)
        else:
            attn_mask = None
        if return_ids:
            ...
            del locs, test_ids
            logits = model(input_ids=ids.to(model.device), attention_mask=attn_mask.to(model.device)).logits
            del ids ; gc.collect()
            return logits
...
def get_workers(params, eval=False):
    tokenizers = []
#    print('Current sys.path is:', sys.path)
